...
do
        ...
        for ((i=1; i <= $cnt; i++)); do
                $BIN -H $SERVER_IP -l $EXEC_TIME -t $TEST_CASE -- -r 100,100 >> $TEST_RESULT_NAME$cnt &
        done
        wait

        echo "Write result to" $TEST_RESULT_NAME$cnt
        cat $TEST_RESULT_NAME$cnt | grep " 100 " | awk '{print $6}' | awk '{sum+=$1} END {print "Netperf stream number =", NR, ", sum =", sum} '
done
